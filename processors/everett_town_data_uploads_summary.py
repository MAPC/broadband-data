...
upload_tests_by_user = combined_data[(combined_data["City"] == "Everett")].groupby(["ip", "date"]).max().reset_index()

chart = alt.Chart(upload_tests_by_user).mark_bar().encode(
    alt.X("MeanThroughputMbps", bin=alt.Bin(extent=[0, 250], step=5)),
    y='count()',
).facet(
    "Year",
    columns=2
).properties(
    title="Everett Upload Speedtests"
)
chart.save('data/finished/everett-upload-counts.html')